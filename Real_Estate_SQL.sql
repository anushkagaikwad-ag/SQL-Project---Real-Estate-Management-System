...
-- Create trigger to check the condition before inserting and updateing
DELIMITER //
...
BEGIN 
     IF EXISTS( 
               SELECT 1 FROM Listing WHERE address = NEW.address AND agentID != NEW.agentID
               )
      THEN 
               SIGNAL SQLSTATE '45000'
               SET message_text = "Cannot insert listing with the same address but different agentID";
	  END IF;
END//
...
/* For checking if trigger is Working properly or not*/
/*INSERT INTO Listing
VALUES('179 Tinker Road', 168, 3001, '2015-09-05');*/


/* queries */
/* 1st query  
"Retrieve the addresses of listings associated with houses. Provide a list of addresses where a listing corresponds to a house." */
SELECT Listing.address
FROM Listing, House
WHERE Listing.address = House.address;

/* 2nd query 
"Retrieve the addresses and MLS numbers of listings associated with houses. Provide details for listings that have a corresponding house."*/
SELECT Listing.address, Listing.mlsNumber
FROM Listing, House
WHERE Listing.address = House.address;

/* 3rd query 
"Find the addresses of listings associated with houses having 3 bedrooms and 2 bathrooms. Include only those listings that match the specified bedroom and bathroom criteria."*/
SELECT Listing.address
FROM Listing, House
WHERE Listing.address = House.address AND House.bedrooms = 3 AND House.bathrooms = 2;

/* 4th query 
"Retrieve addresses and prices of houses with 3 bedrooms, 2 bathrooms, a price between $100,000 and $250,000, and order the results by price in descending order." */
...
/* 5th query  
"Retrieve addresses and prices of business properties of type 'office space'. Order the results by price in descending order."*/
SELECT address, price
...
/* 6th query 
"List the agent ID, name, phone number, firm name, and start date for all agents along with their respective firms."*/
SELECT agentId, Agent.name, phone, Firm.name, dateStarted
FROM Agent, Firm
WHERE Agent.firmId = Firm.id;

/* 7th query 
"Retrieve all details of properties listed by an agent with the ID 201. Include information about the properties associated with this agent." */
SELECT Property.*
FROM Property, Listing
WHERE Property.address = Listing.address AND Listing.agentId = 201;

/* 8th query 
"Provide a list of agent names and corresponding buyer names who have worked together. Include details from the 'Work_With' table connecting agents and buyers."*/
...
/* 9th query
 "Retrieve details of houses matching the preferences of a buyer with ID 799. Include houses with the same number of bedrooms and bathrooms falling within the preferred price range of the buyer." */
SELECT House.*
FROM House, Buyer
